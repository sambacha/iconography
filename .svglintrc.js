  ...
  rules: {
    ...
    custom: [
      ...
      function (reporter, $, ast) {
        ...
        const precisionMax =
          precisionArray && precisionArray.length > 0
            ? Math.max(...precisionArray)
            : 0;

        if (precisionMax > iconMaxFloatPrecision) {
          reporter.error(
            `Maximum precision should not be greater than ${iconMaxFloatPrecision}; it is currently ${precisionMax}`
          );
          if (updateIgnoreFile) {
            ignoreIcon(reporter.name, iconPath, $);
          }
      ...
      function (reporter, $, ast) {
        ...
        if (invalidSegments.length) {
          invalidSegments.forEach(([command, x1Coord, y1Coord, ...rest]) => {
            ...
            if (curveCommands.includes(command)) {
              ...
              if (
                ...
                resolutionTip = `should be "l${removeLeadingZeros(
                  x2Coord
                )} ${removeLeadingZeros(y2Coord)}" or removed`;
              }
              ...
              if (
                ...
                resolutionTip = `should be "l${removeLeadingZeros(
                  xCoord
                )} ${removeLeadingZeros(yCoord)}" or removed`;
              }
            ...
            reporter.error(
              `Ineffective segment "${readableSegment}" in path (${resolutionTip}).`
            );
          });
          if (updateIgnoreFile) {
            ignoreIcon(reporter.name, iconPath, $);
      ...
      function (reporter, $, ast) {
        ...
        getCollinearSegments(iconPath).forEach((segment) => {
          ...
          reporter.error(
            `Collinear segment "${segmentString}" in path (should be removed)`
          );
        });
      },
      function (reporter, $, ast) {
      ...
      function (reporter, $, ast) {
        ...
        if (negativeZeroMatches.length) {
          ...
          const pathDIndex =
            svgFileHtml.indexOf(pathDStart) + pathDStart.length;
